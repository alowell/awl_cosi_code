...
bool AtmosphereEventListCreator::Analyze()
{
  ...
  while ((Event = Reader->GetNextEvent()) != 0) {
    ...
    vector<EventListEntry> E;
    bool LastIAWasENTR = false;
	 bool IsDirect = false;
    ...
    for (unsigned int i = 0; i < Event->GetNIAs(); ++i) {
      ...
      if (IA->GetProcess() == "ENTR") {
        if (LastIAWasENTR == false) {

			 //this will reject "direct" events, i.e. those events that did not interact with the atmosphere before entering a test sphere
			 if(i == 1){
				 IsDirect = true;
				 break;
			 }
          ...
          EventListEntry e;
          e.Position = Position;
          e.Direction = IA->GetMotherDirection();
          e.Polarization = IA->GetMotherPolarisation();
          e.Energy = IA->GetMotherEnergy();
	 ...
	 if(!IsDirect){
		 if(E.size() > 0){
			 ...
			 if(E.size() == 1){
				 ...
				 idx = gRandom->Integer(E.size());
			 }
			 list<<++ID<<" 0  1  0  "<<E[idx].Time<<"  "
				 <<E[idx].Position.X()<<"  "<<E[idx].Position.Y()<<"  "<<E[idx].Position.Z()<<"  "
				 <<E[idx].Direction.X()<<"  "<<E[idx].Direction.Y()<<"  "<<E[idx].Direction.Z()<<"  "
				 <<E[idx].Polarization.X()<<"  "<<E[idx].Polarization.Y()<<"  "<<E[idx].Polarization.Z()<<"  "
				 <<E[idx].Energy<<endl;       
		 }
	 }
